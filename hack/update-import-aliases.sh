...
source "hack/util.sh"
util::install_tools ${GOIMPORTS_PKG} ${GOIMPORTS_VERSION}
...
go run "${ROOT_PATH}/hack/tools/preferredimports/preferredimports.go" -confirm -import-aliases "${IMPORT_ALIASES_PATH}" -include-path "${INCLUDE_PATH}"  "${ROOT_PATH}" || ret=$?
if [[ $ret -ne 0 ]]; then
